(ns cowin.main
  "Kotodama WASM entrypoint for the Cowin clean-room actor (L4) — Clojure port.
  ...
  facts (`cowin.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"cowin.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "cowin-compat" :tier tier :entities entities} 200])
